...
import { listSettlements as listEzypaySettlements, downloadDocument as downloadEzypayDocument } from "@/lib/settlements"
...
export async function listTransactionByInvoice(invoiceId, paymentMethod, branch) {
...
export async function replacePaymentMethod(customerId, paymentMethod, newPaymentMethod, branch) {
  return await replaceEzypayPaymentMethod(customerId, paymentMethod, newPaymentMethod, branch)
}
...
export async function activatePayTo(customerId: string, paymentMethodToken: string, branch: string) {
  return await activateEzypayPayTo(customerId, paymentMethodToken, branch)
}
